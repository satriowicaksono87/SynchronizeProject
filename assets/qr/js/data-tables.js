...
(function (m, q) {
	...
	var q = window,
		...
		F = function (T, U, V) {
			if (B(T))
				for (var W in T) Object.prototype.hasOwnProperty.call(T, W) && U.call(V, T[W], W);
			else
				for (W = 0; W < T.length; W++) U.call(V, T[W], W)
		},
		G = function (T, U, V) {
		...
		L = function (T, U) {
			...
			for (var X = !0; X;) {
				X = !1;
				for (var Y = V; Y != W; Y += U)
					if (T[Y + U] && T[Y].value > T[Y + U].value) {
						var Z = T[Y],
							$ = T[Y + U];
						T[Y] = $, T[Y + U] = Z, X = !0
					}
		...
		M = function (T, U, V, W, X) {
			...
			for (var ca = 1; ca <= V; ca++)
				if (1 == ca || ca == V || ca >= $ && ca <= _) {
					var da = T[ca - 1];
	...
	P.prototype.swap = function (T) {
		...
		var X, Y = W.data,
			...
			_ = 0,
			aa = 0,
			ba = W.activeHeadings[T];
		T = ba.originalCellIndex, F(Y, function (fa) {
			var ga = fa.cells[T],
				ha = ga.hasAttribute("data-content") ? ga.getAttribute("data-content") : ga.data,
				ia = ha.replace(/(\$|\,|\s|%)/g, "");
			if ("date" === ba.getAttribute("data-type") && q.moment) {
				var ja = !1,
					ka = ba.hasAttribute("data-format");
	...
	Q.prototype.build = function (T) {
		...
		if (C(T)) {
			...
			C(T[0]) ? F(T, function (V) {
				U.data.push(this.build(V))
			}, this) : U.data.push(this.build(T)), U.data.length && (U.hasRows = !0), this.update(), U.columns().rebuild()
		}
	...
	return S.init = function (T) {
		...
		if (V.data && N.call(U), V.ajax) {
			var X = V.ajax,
				...
				$ = function (ia) {
					if (4 === Y.readyState)
						if (U.emit("datatable.ajax.loaded", ia, Y), 200 === Y.status) {
							var ja = {};
		...
		if (U.headings = [], U.hasHeadings = 0 < U.head.rows.length, U.hasHeadings && (U.header = U.head.rows[0], U.headings = [].slice.call(U.header.cells)), !V.header && U.head && U.table.removeChild(U.table.tHead), V.footer ? U.head && !U.foot && (U.foot = H("tfoot", {
				html: U.head.innerHTML
			}), U.table.appendChild(U.foot)) : U.foot && U.table.removeChild(U.table.tFoot), U.wrapper = H("div", {
				class: "dataTable-wrapper dataTable-loading"
			}), W += "<div class='dataTable-top'>", W += V.layout.top, W += "</div>", W += "<div class='dataTable-container'></div>", W += "<div class='dataTable-bottom'>", W += V.layout.bottom, W += "</div>", W = W.replace("{info}", "<div class='dataTable-info'></div>"), V.perPageSelect) {
			...
			F(V.perPageSelect, function (ia) {
				var ja = ia === V.perPage,
					ka = new Option(ia, ia, ja, ja);
				ea.add(ka)
			}), da = da.replace("{select}", ea.outerHTML), W = W.replace("{select}", da)
		} else W = W.replace("{select}", "");
		if (V.searchable) {
			var fa = "<div class='dataTable-search'><input id='scanned-QR' class='dataTable-input' placeholder='" + V.labels.placeholder + "' type='text'></div>";
		...
		if (I(this.pagers, this.isIE), 1 < this.totalPages) {
			var T = "pager",
				U = u.createDocumentFragment(),
				V = this.onFirstPage ? 1 : this.currentPage - 1,
		...
		if (this.options.fixedColumns && this.activeHeadings && this.activeHeadings.length) {
			...
			if (this.columnWidths = [], this.table.tHead) F(this.activeHeadings, function (Y) {
				...
				var $ = Y.offsetWidth,
					_ = 100 * ($ / this.rect.width);
				Y.style.width = _ + "%", this.columnWidths[Z] = $
			}, this);
			else {
				T = [], U = H("thead");
				var V = H("tr"),
					W = this.table.tBodies[0].rows[0].cells;
				F(W, function () {
					var Y = H("th");
		...
		return T = T.toLowerCase(), this.currentPage = 1, this.searching = !0, this.searchData = [], T.length ? void(this.clear(), F(this.data, function (V, W) {
			var X = -1 < this.searchData.indexOf(V),
				Y = T.split(" ").reduce(function (Z, $) {
					for (var _ = !1, aa = null, ba = null, ca = 0; ca < V.cells.length; ca++)
						if (aa = V.cells[ca], ba = aa.hasAttribute("data-content") ? aa.getAttribute("data-content") : aa.textContent, -1 < ba.toLowerCase().indexOf($) && U.columns(aa.cellIndex).visible()) {
							_ = !0;
							break
						} return Z && _
				}, !0);
		...
		if (B(T)) {
			...
			var Z = [];
			F(Y, function ($, _) {
				var aa = U.labels.indexOf(_); - 1 < aa && (Z[aa] = $)
			}), V.push(Z)
		});
		...
		var X, Y, Z, $, U = this.activeHeadings,
			V = [],
			W = [];
		if (!B(T)) return !1;
		var aa = E({
		...
		if (aa.type) {
			if (("txt" === aa.type || "csv" === aa.type) && (V[0] = this.header), !aa.selection) V = V.concat(this.activeRows);
			else if (!isNaN(aa.selection)) V = V.concat(this.pages[aa.selection - 1]);
			else if (C(aa.selection))
				for (X = 0; X < aa.selection.length; X++) V = V.concat(this.pages[aa.selection[X] - 1]);
			if (V.length) {
				if ("txt" === aa.type || "csv" === aa.type) {
					for (Z = "", X = 0; X < V.length; X++) {
						for (Y = 0; Y < V[X].cells.length; Y++)
							if (0 > aa.skipColumn.indexOf(U[Y].originalCellIndex) && this.columns(U[Y].originalCellIndex).visible()) {
					...
					for (Y = 0; Y < V.length; Y++)
						for (W[Y] = W[Y] || {}, X = 0; X < U.length; X++) 0 > aa.skipColumn.indexOf(U[X].originalCellIndex) && this.columns(U[X].originalCellIndex).visible() && (W[Y][U[X].textContent] = V[Y].cells[X].textContent);
		...
		var U = 1;
		this.hasRows && (U = this.data[0].cells.length), K.add(this.wrapper, "dataTable-empty"), this.clear(H("tr", {
			html: "<td class=\"dataTables-empty\" colspan=\"" + U + "\">" + T + "</td>"
		}))
		...
		if (this.events = this.events || {}, !1 != T in this.events)
			for (var U = 0; U < this.events[T].length; U++) this.events[T][U].apply(this, Array.prototype.slice.call(arguments, 1))
	}, R
});
